...
O_INIT = np.array([15.685, 12.755, 5.133 + 1.23])
...
def nerf_build_batch(
    ...
    dihed_o_c_n_ca: Union[float, torch.Tensor] = None,
    ...
    if bond_len_n_ca is None:
        bond_len_n_ca = N_CA_LENGTH
    if bond_len_ca_c is None:
        bond_len_ca_c = CA_C_LENGTH
    if bond_len_c_n is None:
        bond_len_c_n = C_N_LENGTH
    if bond_len_o_c is None:
        bond_len_o_c = O_C_LENGTH
    if bond_angle_c_n_ca is None:
        bond_angle_c_n_ca = C_N_CA_ANGLE
    if bond_angle_ca_c_n is None:
        bond_angle_ca_c_n = CA_C_N_ANGLE
    if bond_angle_n_ca_c is None:
        bond_angle_n_ca_c = N_CA_C_ANGLE
    if bond_angle_o_c_n is None:
        bond_angle_o_c_n = O_C_N_ANGLE
    ...
    o_coord = place_dihedral(
            ...
            final_pos[:, 0, : ],
            c_coord,
            bond_angle=bond_angle_o_c_n[:, i].unsqueeze(1),
            bond_length=bond_len_o_c[:, i].unsqueeze(1),
            torsion_angle=dihed_o_c_n_ca[:, i].unsqueeze(1),
            use_torch=True,
        )
    o_coords.append(o_coord)
