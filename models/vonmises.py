...
class VonMisesMix(Distribution):
    def __init__(self, locs, concentrations, log_weights, conc_min=1.0, conc_max=20.0):
        super().__init__(validate_args=False)
        self.locs = locs
        self.concentrations = torch.sigmoid(concentrations) * (conc_max - conc_min) + conc_min
        self.log_weights = log_weights
        mixture_dist = D.Categorical(logits=log_weights)
        component_dist = VonMises(loc=self.locs, concentration=self.concentrations)
        self.mixture_model = MixtureSameFamily(mixture_dist, component_dist)

    def sample(self, sample_num):
        return self.mixture_model.sample((sample_num,))

    def log_prob(self, value):
        log_prob_x = self.mixture_model.component_distribution.log_prob(value)
        mix_logits = self.mixture_model.mixture_distribution.logits
